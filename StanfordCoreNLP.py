...
class StanfordCoreNLP(object):
    ...
    def checkPathHost(self):
        ...
        if len(glob.glob(directory + languageFiles.get(self.lang))) <= 0:
            raise IOError(jars.get(self.lang) + ' not exists. You should download and place it in the ' + directory + ' first.')
...
#************************************************List Of Annotators************************************************#
    ...
    def regexner(self, textValue, patternValue, filterValue = False):
        ...
        #Get the currect Annotator data from the server     
        requestedDictValue = self.getDataForAnnotatorsWrapper(self.url + '/regexner', 'regexner', textValue, patternValue = patternValue, filterValue = filterValue)
        return requestedDictValue
    ...
    def tokensregex(self, textValue, patternValue, filterValue = False):
        ...
        #Get the currect Annotator data from the server
        requestedDictValue = self.getDataForAnnotatorsWrapper(self.url + '/tokensregex', 'tokenize, ssplit, depparse', textValue, patternValue = patternValue, filterValue = filterValue)
        return requestedDictValue
    ...
    def tregex(self, textValue, patternValue, filterValue = False):
        ...
        #Get the currect Annotator data from the server
        requestedDictValue = self.getDataForAnnotatorsWrapper(self.url + '/tregex', 'tokenize, ssplit, depparse, parse', textValue, patternValue = patternValue, filterValue = filterValue)
        return requestedDictValue
    ...
    def semgrex(self, textValue, patternValue, filterValue = False):
        ...
        #Get the currect Annotator data from the server
        requestedDictValue = self.getDataForAnnotatorsWrapper(self.url + '/semgrex', 'tokenize, ssplit, depparse', textValue, patternValue = patternValue, filterValue = filterValue)
        return requestedDictValue
    ...
    def openie(self, textValue):
        ...
        #Get the currect Annotator data from the server
        requestedDictValue = self.getDataForAnnotatorsWrapper(self.url, 'tokenize, ssplit, pos, lemma, depparse, natlog, openie', textValue)
        returnDictList = []
        #Go through all the Constituency Parsing in each sentence in order to get the Constituency Parsing information
        for s in requestedDictValue['sentences']:
            returnDictList.append(s['openie'])
    ...
    def kbp(self, textValue):
        ...
        #Get the currect Annotator data from the server
        requestedDictValue = self.getDataForAnnotatorsWrapper(self.url, 'tokenize, ssplit, pos, lemma, ner, parse, coref, kbp', textValue)
        returnDictList = []
        #Go through all the Knowledge Base Population in each sentence in order to get the Knowledge Base Population information
        for s in requestedDictValue['sentences']:
            returnDictList.append(s['kbp'])
    ...
    def quote(self, textValue):
        ...
        #Get the currect Annotator data from the server
        requestedDictValue = self.getDataForAnnotatorsWrapper(self.url, 'tokenize, ssplit, pos, lemma, ner, depparse, coref, quote', textValue)
        returnDictList = []
        #Go through all the Knowledge Base Population in each sentence in order to get the Knowledge Base Population information
    ...
    def truecase(self, textValue):
        ...
        #Get the currect Annotator data from the server
        requestedDictValue = self.getDataForAnnotatorsWrapper(self.url, 'tokenize, ssplit, truecase, pos, lemma, ner', textValue)
        returnDictList = []
        #Go through all the tokens, span and their truecase in the text
        #In order to rearrange them in the Dictionary List
        for s in requestedDictValue['sentences']:
